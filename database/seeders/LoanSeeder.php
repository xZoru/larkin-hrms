        ...
        foreach ($employees as $index => $employee) {
            ...
            $loan = Loan::updateOrCreate(
                ...
                [
                    ...
                    'reason' => 'Sample staff loan record',
                    'status' => $status,
                    'created_by' => 1,
                ]
            );
            ...
            if (in_array($status, ['Approved', 'Released', 'On-Hold', 'Completed'])) {
                $loan->update([
                    'approved_by' => 1,
                    'approved_date' => Carbon::now()->subDays(rand(1, 30)),
                ]);
            }
            ...
            if (in_array($status, ['Released', 'Completed'])) {
                $loan->update([
                    'released_by' => 1,
                    'released_date' => Carbon::now()->subDays(rand(1, 15)),
                ]);
            }

            if (in_array($status, ['Released', 'Completed'])) {
                $paymentsMade = $status === 'Completed' ? $installmentCount : rand(1, max(1, $installmentCount - 1));
                $paidAmount = 0;
