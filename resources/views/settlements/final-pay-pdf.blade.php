...
<table class="copies"><tr>@for($copy = 0; $copy < 2; $copy++)<td>
    ...
    <table class="employee-details">
        ...
        <tr><td class="label">Hours per day:</td><td class="value">{{ number_format($payment->hours_per_day, 2) }}</td></tr>
    ...
    <div class="calculation">Calculation:<table class="calc"><tr><td class="operator"></td><td class="figure">1.5</td><td class="description"></td></tr><tr><td class="operator">x</td><td class="figure">{{ number_format($payment->hours_per_day, 2) }}</td><td class="description">Hours/day</td></tr><tr><td class="operator">x</td><td class="figure">{{ number_format($payment->hourly_rate, 2) }}</td><td class="description">Hourly rate</td></tr><tr><td class="operator">x</td><td class="figure total">{{ number_format($payment->service_months, 2) }}</td><td class="description">Months in Service</td></tr><tr><td class="operator"></td><td class="figure total">K {{ number_format($payment->amount, 2) }}</td><td class="description"></td></tr></table></div>
    <div class="signature"><div class="issued"><b>Issued by:</b> <span class="name">{{ strtoupper($payment->issuer_name) }}</span><br><span class="issuer-position">{{ $payment->issuer_position }}</span></div><div class="signature-row"><b>Received by:</b> <span class="line">{{ $payment->employee->full_name }}</span></div><div class="signature-row"><b>Date:</b> <span class="line">&nbsp;</span></div></div>
